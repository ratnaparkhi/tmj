...
def insertRecords(jRecs, cRecs, lRecs, tRecs, dbCon):
    jRecTempl = ','.join(['%s'] * len(jRecs))
    jComTempl = ','.join(['%s'] * len(cRecs))
    jLvlTempl = ','.join(['%s'] * len(lRecs))
    jTagTempl = ','.join(['%s'] * len(tRecs))
    if len(jRecs) > 0:
        try:
            qry = 'insert into tm_jobs1 values {0}'.format(jRecTempl)
            dbCursor = dbCon.cursor()
            dbCursor.execute(qry, jRecs)
        except psycopg2.Error as err:
            print ("Function insertRecords: Unable to execute 'insert into tm_jobs1")
            print(err)
            exit(1)
        dbCon.commit()
    if len(cRecs) > 0:
        try:
            qry = 'insert into tm_companies1 values {0}'.format(jComTempl)
            dbCursor = dbCon.cursor()
            dbCursor.execute(qry, cRecs)
        except psycopg2.Error as err:
            print ("Function insertRecords: Unable to execute 'insert into tm_companies1")
            print(err)
            exit(1)
        dbCon.commit()
    if len(lRecs) > 0:
        try:
            qry = 'insert into tm_levels1 values {0}'.format(jLvlTempl)
            dbCursor = dbCon.cursor()
            dbCursor.execute(qry, lRecs)
        except psycopg2.Error as err:
            print ("Function insertRecords: Unable to execute 'insert into tm_levels1")
            print(err)
            exit(1)
        dbCon.commit()
    if len(tRecs) > 0:
        try:
            qry = 'insert into tm_tags1 values {0}'.format(jTagTempl)
            dbCursor = dbCon.cursor()
            dbCursor.execute(qry, tRecs)
        except psycopg2.Error as err:
            print ("Function insertRecords: Unable to execute 'insert into tm_tags1")
            print(err)
            exit(1)
        dbCon.commit()
...
def main(argv):
    ...
    if (pages == 1):
        insertRecords(jobRecs, comRecs, lvlRecs, tagRecs, dbConn)
    elif (pages != 'All'):
